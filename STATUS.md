...
**Changed:** נבנו ארבעת הכלים שאושרו מהביקורת: **`listings.py` — חיתום בכמות
...
| | |
|---|---|
| **Status** | 🟢 הכל עובד. ARV ±12.6% מאומת · `listings.py` חותם CSV של Redfin. RentCast לא נבדק (אין מפתח) |
...
- `listings.py` — **חיתום בכמות.** CSV של Redfin (או RentCast) → כל המודעות
  ב-BUY BOX ממוינות לפי הפער מהצעת ה-70%. `.env.example` — שם המפתח האופציונלי
