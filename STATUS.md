...
**Changed:** יושמו ההחלטות מחלק ג' של הביקורת: **כלל ההצעה 72%** (היה 70%,
...
🔴 **פוש נכשל:** `github.com/vanmtomer-newway/New-Way` לא קיים או לא נגיש
ל-`tomer-collab`. הקומיטים מקומיים בלבד.
...
| | |
|---|---|
| **Status** | 🟢 הכל עובד על 12 זיפי היעד. ARV לפי שכבה: BUY BOX ±12.4%, צפון ±18.7% · כלל 72% · `listings.py` חותם CSV של Redfin. RentCast לא נבדק (אין מפתח). 🔴 הריפו המרוחק לא נגיש — הכל מקומי |
...
# 👇 חיתום בכמות — מודעות פעילות מ-CSV של Redfin ("Download All" בתחתית החיפוש)
python3 listings.py data/redfin/*.csv --dom 90              # רק מודעות תקועות, ממוין לפי הפער מהצעת ה-70%
